...
async function cancel(userId, appId) {
    ...
    if (employer)
        await prisma.notification.create({ data: { userId: employer.userId, type: 'application.cancelled', title: 'Application cancelled', content: `Employer cancelled application for "${job.title}"` } });
    return updated;
}
...
async function apply(userId, body) {
    ...
    const employer = await prisma.employerProfile.findUnique({ where: { id: job.employerId } });
    if (employer)
        await prisma.notification.create({ data: { userId: employer.userId, type: 'application.pending', title: 'New application', content: `A worker applied to "${job.title}"` } });
    return created;
}
